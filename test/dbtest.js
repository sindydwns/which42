...
async function test2() {
	await sequelize.sync({force: true});
	const yonshin = await User.create({intraId: "yonshin", slackId: "AAAAAAAA"});
	const hyeyukim = await User.create({intraId: "hyeyukim", slackId: "BBBBBBBB"});
	const sanghwal = await User.create({intraId: "sanghwal", slackId: "CCCCCCCC"});
	
	// groupFuncs.insertGroup();
	await groupFuncs.insertGroup(yonshin.intraId, "yonshin group 1");
	const yonshinGroup1 = await Group.findOne({ where: {intraId: "yonshin"}});
	...
	assert.equal((await Group.findOne({ where: {intraId: "yonshin"}})).name, "yonshin group 1");
	await groupFuncs.insertGroup(hyeyukim.intraId, "hyeyukim group 1");
	const hyeyukimGroup1 = await Group.findOne({ where: {intraId: "hyeyukim"}});
	...
	assert.equal((await Group.findOne({ where: {intraId: "hyeyukim"}})).name, "hyeyukim group 1");
	await groupFuncs.insertGroup(yonshin.intraId, "yonshin group 2"); // yonshin has two group
	const yonshinGroup2 = await Group.findOne({ where: {name: "yonshin group 2"}});
	...
	assert.equal((await Group.findOne({ where: {intraId: "hyeyukim"}})).name, "hyeyukim group 1");
	assert.deepEqual(
		(await Group.findAll({ where: {intraId: "yonshin"}})).map(x => x.groupId).sort(),
		[yonshinGroup1.groupId, yonshinGroup2.groupId].sort()
	);
	assert.deepEqual(
		(await Group.findAll({ where: {intraId: "yonshin"}})).map(x => x.name).sort(),
	...
	assert.equal(
		(await Group.findOne({ where: {selected: true, intraId: yonshin.intraId, groupId: yonshinGroup1.groupId}})).name,
		"yonshin group 1"
	);
	...
	assert.equal(
		(await Group.findOne({ where: {selected: true, intraId: hyeyukim.intraId, groupId: hyeyukimGroup1.groupId}})).name,
		"hyeyukim group 1"
	);
	await groupFuncs.updateSelectedGroup(yonshin.intraId, yonshinGroup2.groupId);
	assert.equal((await Group.findAll({ where: {selected: false}})).length, 1);
	assert.equal(
		(await Group.findOne({ where: {selected: true, intraId: yonshin.intraId, groupId: yonshinGroup2.groupId}})).name,
		"yonshin group 2"
	);
	...
	assert.equal((await groupFuncs.getSelectedGroupId(yonshin.intraId)), yonshinGroup2.groupId);
	assert.equal((await groupFuncs.getSelectedGroupId(hyeyukim.intraId)), hyeyukimGroup1.groupId);
	...
	assert.deepEqual(
		...
		[{targetId:"yonshin"}, {targetId:"hyeyukim"}, {targetId:"sanghwal"}, {targetId:"ekwak"}].sort(sortByTargetId)
	);
	assert.deepEqual(
	...
	assert.deepEqual(
		(await groupFuncs.getMemberList(yonshinGroup2.groupId)).sort(sortByTargetId),
...
async function test3() {
	await sequelize.sync({force: true});
	const yonshin = await User.create({intraId: "yonshin", slackId: "AAAAAAAA"});
	const sanghwal = await User.create({intraId: "sanghwal", slackId: "BBBBBBBB"});
	...
	assert.equal((await Alarm.findAll({ where: {targetId: ekwak.intraId}})).length, 1);
	assert.equal((await Alarm.findAll({ where: {targetId: yonshin.intraId}})).length, 2);
